...
public class PrimsMST {
	...
	public void PrimeMST(PrimsMST graph) {
		...
		while (heap.size() != 0) {
			int minEdgeVertex = findMin(); // get the vertex with minimum edge
											// in the heap
			heap.remove(minEdgeVertex); // node removed from heap and its moves
										// to the Set s.
	...
	/**
	 * 
	 * @return the minimum key for a given value here key is "node" and value is
	 *         "weight" of all edges in the heap
	 */
	private int findMin() {
